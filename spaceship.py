...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            ...
            canvas.draw_image(self.image, [self.image_center[0]*self.age,self.image_center[1]*1], self.image_size,
                          self.pos, self.image_size, self.angle)
        canvas.draw_image(self.image, self.image_center, self.image_size,
                          self.pos, self.image_size, self.angle)
    def collide(self,other_object):
        ...
        x=(other_pos[0]-self.pos[0])*(other_pos[0]-self.pos[0]) 
        y=(other_pos[1]-self.pos[1])*(other_pos[1]-self.pos[1]) 
        distance=math.sqrt(x+y)
        if(distance <= other_radius+self.radius and distance!="nan"):
            return True
        else:
            return False
...
def group_group_collide(group,other_group):
    ...
    for sprite in copy_group:
        if(group_collide(copy_other_group,sprite)):
            collision_num+=1
